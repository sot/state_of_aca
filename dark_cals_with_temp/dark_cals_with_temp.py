...
def main(opt):
    ...
    for i in range(0, len(dirs)):
     ...
     #   imgs[i] = hdus[0].data
        ...
        #nf = plt.figure(figsize=(5,5))
        #plt.plot((img > limit_z), 'k.')
        #plt.xlim(0,1024)
        #plt.ylim(0,1024)
        #plt.savefig('%s_dist.png' % day)
        #plt.close(nf)
    ...
    f = open('dark_cals_with_temp.txt', 'w')
    f.write(",".join(warm_pix.dtype.names))
    f.write("\n")
    for i in range(0, len(warm_pix)):
        f.write("{}\n".format(",".join([str(n) for n in warm_pix[i].tolist()])))
    f.close()
